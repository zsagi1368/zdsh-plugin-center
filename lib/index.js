...
import { accessSync, appendFileSync, closeSync, constants, copyFileSync, existsSync, lstatSync, mkdirSync, openSync, readFileSync, readdirSync, renameSync, rmSync, statSync, unlinkSync, writeFileSync, writeSync } from "node:fs";
...
function isHostAllowed(hostname) {
	...
	if (host.includes(":")) {
		...
		if (first === 100 && hex[1] === 65435 && (hex[2] === 0 || hex[2] === 1) && hex.slice(3, 6).every((n) => n === 0)) return judgeIpv4Literal(hexGroupsToIpv4(hex[6], hex[7]));
...
*/
async function assertHostResolvesAllowed(hostname) {
	...
	for (const entry of addresses) if (!isHostAllowed(entry.address)) return {
		ok: false,
		error: {
			code: CpErrorCode.unsafeUrl,
...
*/
async function readBodyBounded(response, maxBytes, stop) {
	...
	if (reader === void 0) {
		...
		return {
			ok: true,
			data: new TextDecoder("utf-8", { fatal: false }).decode(buffer)
		};
	}
	const chunks = [];
	let received = 0;
	...
	return {
		ok: true,
		data: new TextDecoder("utf-8", { fatal: false }).decode(merged)
	};
}
/**
...
async function safeFetch(rawUrl, options = {}) {
	...
	for (let hop = 0; hop <= maxRedirects; hop += 1) {
		...
		if (current.data.protocol !== "https:") return {
			ok: false,
			error: {
				code: CpErrorCode.unsafeUrl,
		...
		if (current.data.origin !== origin) {
			const sanitized = {};
			for (const [key, value] of Object.entries(headers)) if (!SENSITIVE_REDIRECT_HEADERS.test(key)) sanitized[key] = value;
			headers = sanitized;
...
function textOk(value) {
	...
	const t = value;
	return typeof t.zh === "string" && t.zh.length >= 1 && t.zh.length <= TEXT_LIMIT && typeof t.en === "string" && t.en.length >= 1 && t.en.length <= TEXT_LIMIT;
}
...
*/
function createPlan(entry, action, profile, profileDir) {
	...
	if (!isValidProfileName(profile)) throw new CpError(CpErrorCode.invalidPlan, `profile must be a bare name for \`dsh plugin --profile\`, got ${JSON.stringify(profile)}`);
	if (!profileDir.trim()) throw new CpError(CpErrorCode.invalidPlan, "profileDir is required");
	return {
		planId: `${hashPlan({
			action,
			profile,
...
function confirmationPhrase(plan) {
	return `确认 ${plan.action === "install" ? "安装 install" : plan.action === "update" ? "更新 update" : "卸载 uninstall"} ${plan.entry.id} @${plan.confirmCode} / confirm`;
}
...
var PlanStore = class {
	...
	add(plan) {
		...
		if (existing !== void 0) {
			if (!(existing.state === "restart-pending" || existing.state === "rolled-back")) throw new CpError(CpErrorCode.invalidPlan, `plan ${plan.planId} already exists`);
		}
	...
	confirm(planId, phrase) {
		...
		if (record.state !== "planned") throw new CpError(CpErrorCode.planConsumed, `plan ${planId} was already confirmed`);
...
async function resolveExecutablePath(name) {
	...
	let result;
	try {
		result = spawnSync("which", [trimmed], {
			encoding: "utf8",
			windowsHide: true
		});
	} catch (error) {
...
function runViaSpawn(spec) {
	return new Promise((resolvePromise, rejectPromise) => {
		...
		Promise.all([import("node:child_process"), planCommand(spec.cmd, spec.args)]).then(([{ spawn }, invocation]) => {
...
*/
function buildInstallCmd(profileName, owner, repo, commit) {
	return {
		cmd: "dsh",
		args: [
			"plugin",
...
function buildNpmAddCmd(profileName, pkgName, version) {
	return {
		cmd: "dsh",
		args: [
			"plugin",
...
function buildRemoveCmd(profileName, pkgName) {
	return {
		cmd: "dsh",
		args: [
			"plugin",
...
var LifecycleEngine = class {
	...
	*/
	buildPlan(entry, action, profileName, profileDir, targetManifest) {
		try {
	...
	*/
	...
	async applyPlanLocked(planId) {
		...
		try {
			...
			const finalState = rolledBack ? "rolled-back" : "executing";
			...
			if (backup && !rolledBack) return cpErr("rollback_failed", `execution failed (${code}) and rollback could not be verified`);
			return toCpResult(error);
		}
	}
...
*/
function resolveDataRoot(config, env = process.env) {
	if (config?.dataRoot) return isAbsolute(config.dataRoot) ? config.dataRoot : resolve(config.dataRoot);
	const branchHome = env.DSH_BRANCH_HOME;
	if (branchHome !== void 0 && branchHome.trim().length > 0) return join(resolve(branchHome), "plugin-center");
	const dshHome = env.DSH_HOME;
	if (dshHome !== void 0 && dshHome.trim().length > 0) return join(resolve(dshHome), "zdsh", "plugin-center");
	return join(homedir(), ".zdsh-plugin-center");
}
/** Storage home the host CLI resolves profiles against (config → env → defaults). */
function profileHome(config) {
	return config.dshHome ?? process.env.DSH_BRANCH_HOME ?? process.env.DSH_HOME ?? defaultDshHome();
}
/**
...
function normalizeConfig(raw = {}) {
	...
	return {
		...
		launchCommand: cfg.launchCommand && typeof cfg.launchCommand === "object" ? {
			cmd: typeof cfg.launchCommand.cmd === "string" && cfg.launchCommand.cmd !== "" ? cfg.launchCommand.cmd : "dsh",
			args: Array.isArray(cfg.launchCommand.args) ? cfg.launchCommand.args.filter((arg) => typeof arg === "string") : ["web"]
		} : void 0,
...
var PluginCenterServices = class {
	...
	*/
	async stagePlan(action, entryId) {
		if (resolveProfileDir(this.config) !== cliProfileDir(this.config)) return cpErr("invalid_plan", `profileDir override diverges from the directory the dsh CLI resolves for profile "${this.config.defaultProfile}"; align it via dshHome instead`);
		const entry = await this.entryById(entryId);
		if (!entry.ok) return entry;
		const built = this.engine.buildPlan(entry.data, action, this.config.defaultProfile, this.profileDir);
	...
	async guardianToggle(action) {
		...
		if (!started.ok) return cpErr("internal", started.reason ?? "guardian start failed");
		return cpOk(started);
	}
	...
	applyRestore(restoreId, code) {
		const staged = this.restores.get(restoreId);
		if (staged === void 0) return cpErr("plan_not_found", "unknown or expired restore request");
		if (Date.now() > staged.expiresAtMs) {
			this.restores.delete(restoreId);
			return cpErr("plan_not_found", "restore request expired");
		}
		if (code !== staged.code) return cpErr("confirmation_mismatch", "confirmation code does not match");
		this.restores.delete(restoreId);
...
function statusFor(code) {
	switch (code) {
		...
		case "unsafe_url": return 400;
		case "plan_not_found": return 404;
		case "plan_consumed":
...
*/
function hostGateOk(headers) {
	const host = headers.host;
	if (!host) return false;
	const bare = (host.startsWith("[") ? host.slice(0, host.indexOf("]") + 1) : host.split(":")[0] ?? "").toLowerCase();
...
async function handleApiRequest(services, request) {
	...
	if (!hostGateOk(request.headers ?? {})) return json(403, { error: {
	...
	if (matches(request, "POST", ROUTES.backupRestoreApply)) {
		const body = request.body ?? {};
		if (!body.restoreId || !body.code) return json(400, { error: {
			code: "invalid_plan",
			message: "restoreId and code are required"
		} });
...
async function readAuditTail(dataRoot, maxLines = 200) {
	...
	try {
		...
		const start = Math.max(0, stats.size - 131072);
		truncated = start > 0;
		const handle = await open(logPath, "r");
		try {
			const length = Math.min(stats.size - start, 131072);
...
async function serveRequest(services, req, res) {
	...
	const parsed = method === "POST" ? await readJsonBody(req) : {
		ok: true,
		body: void 0
	};
	if (!parsed.ok) {
